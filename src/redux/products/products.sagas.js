...
export function* fetchProductsAsync({payload: {id}}) {
    try {
        const response = yield Axios.get(`http://backend.comptoir-d-orient.fr/api/products/${id}/edit`);
...
export function* fetchAllProductsAsync() {
    try {
        const response = yield Axios.get(`http://backend.comptoir-d-orient.fr/api/products`);
        const Products = response.data.products
...
export function* getProductAsync({payload: {id}}) {
    try {
        const response = yield Axios.get(`http://backend.comptoir-d-orient.fr/api/products/${id}`);
